...
frame2.grid(row = 1)
# frame2.config(width = 1000, height = 500)

label1 = tk.Label(frame1, text = 'label1 in frame1')
label1.grid(row = 0)

canvas = tk.Canvas(frame2)
canvas.grid(row = 0, column = 0)
vsb = tk.Scrollbar(frame2, orient = 'vertical', command = canvas.yview)
vsb.grid(row = 0, column = 1, sticky = 'ns')
canvas.configure(yscrollcommand = vsb.set)
hsb = tk.Scrollbar(frame2, orient = 'horizontal', command = canvas.xview)
hsb.grid(row = 1, column = 0, sticky = 'we')
canvas.configure(xscrollcommand = hsb.set)
...
with open('keys.csv') as password_file:
	for i, row in enumerate(password_file):
		tk.Radiobutton(frame3).grid(row = i, column = 0)
		a, b, c, d = row.strip().split(',')
		tk.Label(frame3, text = a).grid(row = i, column = 1)
		tk.Label(frame3, text = b).grid(row = i, column = 2)
		tk.Label(frame3, text = c).grid(row = i, column = 3)
frame3.update_idletasks()
# frame3.config(width = 5000, height = 500)
